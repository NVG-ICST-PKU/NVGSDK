...
#define FAIL(ERR) { ret = ERR; goto end; }
    ...
    lavfi->orientation_cur=lavfi->graph->nb_filters/2; //sj add
    lavfi->orientation_pre=lavif->orientation_cur;     //sj add
...
static int vr_read_packet(AVFormatContext *avctx, AVPacket *pkt)
{
	...
	*/
	...
	//sj add
    if(lavfi->orientation_cur!=lavfi->orientation_pre)
    {
    	if(lavfi->orientation_cur<0||lavfi->orientation_cur>=lavfi->graph->nb_filters)
    		return AVERROR_EOF;
    	for(int i=0;i<lavfi->graph->nb_filters;i++)
    	{
			if(i==lavfi->orientation_cur)
    		    av_opt_set_int(lavfi->graph->filters[i]->priv, "rate_zhd", 1, 0);
    		else 
    			av_opt_set_int(lavfi->graph->filters[i]->priv, "rate_zhd", 0, 0);
		}
